...
const pick = document.querySelector("#horse");
...
btn.addEventListener("click", () =>{
    console.log('start')
    if(pick.value <5 && pick.value >0){
        if(money.value >=100){
            pickHorse = pick.value;
            bettingMoney = money.value;
            gameStart = true;
        ...
        if(rank == 4){
            ...
            for(let i = 0; i < horseList.length; i++){
                if(horseList[i].rank ==1){
                    rankOne = horseList[i].line;
                }
            }
            if( rankOne == 0){
                rank.innerHTML = `
                Winner <img src="doux1.png" alt="">
            `;
            } else if( rankOne == 1){
                rank.innerHTML = `
                Winner <img src="mort1.png" alt="">
            `;
            } else if( rankOne == 2){
                rank.innerHTML = `
                Winner <img src="tard1.png" alt="">
            `;
            } else if( rankOne == 3){
                rank.innerHTML = `
            ...
            if(rankOne == pickHorse-1){
                ment.innerHTML = `WIN!!!!    당첨금 : ${Math.floor(bettingMoney*3.9)}원`;
            } else {
                ment.innerHTML = "You LOSE~~~~~~";
            }
